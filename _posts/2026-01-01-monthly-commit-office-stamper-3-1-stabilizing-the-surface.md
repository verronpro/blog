...
The goal of this release is simple: reduce the "surface area" of the library to
ensure that future improvements don't break user code.
...
Leaving these exposed creates a "technical debt of expectations." Users might
start relying on them, making it impossible for me to refactor the internals
...
I've removed or made private over a dozen internal utility classes. These were
effectively "implementation leaks." By hiding them, I've reduced the library
public footprint, making it easier for new users to find the classes they
...
This change aligns with the "Pipeline" philosophy I've been advocating for:
stamping is just one step in a document's lifecycle. Post-processors allow you
to stack operations (cleaning tags, removing orphaned notes, etc.) in a clear,
declarative way.
...
* **For Me**: The freedom to refactor the core engine without fearing a cascade
  of breaking changes across the ecosystem.
...
- **Breaking**: Replaced legacy `cleanTags` methods with
  `Postprocessors.removeTags(String)`.
- **Refactor**: Hidden or removed internal utility classes (`SectionUtil`,
  `TagsVisitor`, etc.).
- **Refactor**: Refined logging levels, shifting verbose engine traces to
  `TRACE`.
