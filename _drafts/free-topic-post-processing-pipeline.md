...
## Thesis: Stamping is not the end
...
In `DocxStamper`, we’ve introduced a configurable pipeline. You can now register a list of post-processors that run sequentially after the core engine has finished its work.

```java
var config = new DocxStamperConfiguration()
    .addPostProcessor(new RemoveProcessorComments())
    .addPostProcessor(new PruneOrphanedFootnotes())
